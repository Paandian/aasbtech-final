...
exports.create = (req, res) => {
  ...
  Bulletin.create({
    ...
    .then(() => {
      res.send({ message: "Bulletin was created successfully!" });
    })
...
exports.update = (req, res) => {
  ...
  Bulletin.update(req.body, {
    ...
    .then((num) => {
      if (num == 1) {
        res.send({
          message: "Bulletin was updated successfully.",
        });
      } else {
        res.send({
          message: `Cannot update Bulletin with id=${id}. Maybe Bulletin was not found or req.body is empty!`,
        });
      }
    })
    .catch((err) => {
...
exports.deleteAll = (req, res) => {
  Bulletin.destroy({
    ...
    .then((nums) => {
      res.send({ message: `${nums} Bulletins were deleted successfully!` });
    })
    .catch((err) => {
      res.status(500).send({
        message:
